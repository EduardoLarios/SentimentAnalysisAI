...
namespace SentimentAnalysis
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var training = File.ReadAllLines("Tweets/training.txt").Select(line => line.ToLower());
...
@DATA";
            ...
            {
                trainingSet.Add($"{ score.Item1[i] },{ score.Item2[2] },{ multipliers.Item1[i] },{ multipliers.Item2[i] },{ results[i] }");
            }

            File.WriteAllLines("training_sentiment.arff", trainingSet);

            Console.WriteLine("Training set is ready");
            Console.WriteLine("\nPress enter to exit...");
            Console.ReadKey();
            var output = "java weka.classifiers.meta.RandomCommittee -l sentiment.model -p 0 -T test.arff > prediction.arff".Bash();
        }
    }
}